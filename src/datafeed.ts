...
// chart-side consumer (the price pane, symbol search, symbol metadata, the countdown) speaks THIS
// interface and never a concrete client, so a third party can drive the chart from their own feed by
...
export interface ChartDatafeed {
  ...
  /** Live bar push for one (symbol, timeframe). Returns the unsubscribe. The transport owns
