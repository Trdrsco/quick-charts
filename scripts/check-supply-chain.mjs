...
export const PACKAGE = 'packages/chart'
...
export const EXCLUSIONS = [
  ['packages/chart/test/boundary/noDefaultTrdrsUrl.test.ts', 'private-host', 'the fixture names the hosts it hunts'],
  ['packages/chart/test/boundary/noDefaultTrdrsUrl.test.ts', 'secret', 'the fixture names the credential shapes it hunts'],
...
const PACKAGE_DOC = /^packages\/chart\/[^/]+\.md$/
...
export function packedFiles(root) {
  ...
  const json = execSync('npm pack --dry-run --json', { cwd: join(root, PACKAGE), encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] })
...
function rulesFor(file, shipped, inHistory = false) {
  return RULES.filter((r) => {
    ...
    if (file === `${PACKAGE}/package.json`) return false
...
export function check(root, options = {}) {
  ...
  // The working tree.
  const files = [...walk(root, PACKAGE), ...walk(root, EXAMPLES)]
  for (const file of files) judge(file, readFileSync(join(root, file), 'utf8'), file.startsWith(`${PACKAGE}/dist/`))
  ...
  if (existsSync(manifestPath)) {
    ...
    for (const block of INSTALLABLE) {
      for (const name of Object.keys(manifest[block] ?? {})) {
        if (rule.re.test(`${name}/`)) report(`${PACKAGE}/package.json`, 0, rule, `${block} names ${name}`, `${block}: ${name}`)
      }
    }
  }
  ...
  for (const [key, lines] of byFile) {
    ...
    const rules = rulesFor(file, file.startsWith(`${PACKAGE}/dist/`), true)
